import { transcribeInShortWindows } from "../lib/asrWindowing.js";
...
async function transcribe({ requestId, audioBuffer, preferredLanguage, modelId, wordTimestamps }) {
  ...
  const output = await transcribeInShortWindows(transcriber, audio, transcriptionOptions, {
    onProgress: (fraction) => postProgress(requestId, Math.round(74 + fraction * 24), `${languageResult.detected ? "检测为" : "按"} ${languageLabel} 转写字幕`),
  });
  self.postMessage({
    type: "result",
    requestId,
