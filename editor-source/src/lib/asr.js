import { transcribeInShortWindows } from "./asrWindowing.js";
...
function transcribeAudioInWorker(audio, { onProgress, preferredLanguage, wordTimestamps }) {
...
async function transcribeAudioOnMainThread(audio, { onProgress, preferredLanguage, wordTimestamps }) {
  ...
  const output = await transcribeInShortWindows(transcriber, audio, transcriptionOptions, {
    onProgress: (fraction) => onProgress?.({progress: Math.round(74 + fraction * 24), phase: `${languageResult.detected ? "检测为" : "按"} ${languageLabel} 转写字幕`}),
  });
...
export async function transcribeAudioToCaptionSegments(
  blob,
  { onProgress, preferredLanguage = "zh", timelineOffset = 0, portraitCaptions = false } = {},
) {
  ...
  let result;
  try {
    result = await transcribeAudioInWorker(audio, { onProgress, preferredLanguage, wordTimestamps: portraitCaptions });
    ...
    result = await transcribeAudioOnMainThread(audio, { onProgress, preferredLanguage, wordTimestamps: portraitCaptions });
  }
  ...
  const rawSegments = portraitCaptions ? groupTimedCaptionWords(result.output, {
    duration, timelineOffset: Math.max(0, timelineOffset || 0), language: result.language,
  }) : outputToCaptionSegments(
    result.output,
    duration,
    Math.max(0, timelineOffset || 0),
    result.language,
  );
  // Word alignment is the timing evidence. Do not apply phrase-level energy
  // heuristics, minimum durations or estimated speech rates to those boundaries.
  const segments = portraitCaptions ? rawSegments : refineCaptionSegmentsWithAudioEnergy(
    rawSegments,
    audio,
    duration,
    Math.max(0, timelineOffset || 0),
