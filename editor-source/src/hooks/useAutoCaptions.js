...
import { extractAudioFromVideo, sliceAudioBlob } from "../lib/media.js";
import { transcribeCaptionSources } from "../lib/autoCaptionSources.js";
...
export function useAutoCaptions(d) {
  return useCallback(async (options = {}) => {
    const sources = options.blob ? [{
      ...options,
      start: Number.isFinite(options.start) ? options.start : 0,
      playbackRate: options.playbackRate || 1,
      sourceDuration: Number.isFinite(options.duration) ? options.duration * (options.playbackRate || 1) : undefined,
    }] : (d.captionSources || []);
    const timelineOffset = sources[0]?.start ?? 0;
    if (d.status === "generating" || d.status === "captioning") return;
    if (d.trackLocks.caption) return void d.notify(d.t("captionTrackLocked"));
    if (!sources.length) return void d.notify(d.t("autoCaptionsNeedsSource"));
    ...
    try {
      const result = await transcribeCaptionSources(sources, {
        extractAudio: extractAudioFromVideo, sliceAudio: sliceAudioBlob, transcribe: transcribeAudioToCaptionSegments,
        preferredLanguage: d.uiLanguage,
        portraitCaptions: d.portraitCaptions,
      ...
      console.error(error);
      const alignmentUnavailable = error?.code === "CAPTION_WORD_ALIGNMENT_REQUIRED"
        || /alignment_heads|cross attentions|token-level timestamps/i.test(error?.message || "");
      const message = alignmentUnavailable
        ? "O modelo não forneceu tempos por palavra. As legendas anteriores foram mantidas; não foram criados tempos estimados."
        : error instanceof Error ? error.message : d.t("autoCaptionsFailed");
      d.setStatus("error"); d.setStatusText(message);
      d.setProgress(0); d.notify(alignmentUnavailable ? message : d.t("autoCaptionsFailedHint"));
    }
  }, [d]);
}
