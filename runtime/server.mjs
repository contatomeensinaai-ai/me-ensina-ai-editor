...
export async function createRuntimeServer({port=5201,distDirectory=fileURLToPath(new URL('../editor/dist/',import.meta.url)),dataDirectory=runtimeDataDirectory(),keywordRun,imageRun,planRun}={}){
 ...
 const server=createServer((req,res)=>{
  ...
  if(['/api/health','/api/status'].includes(path)){
   ...
   res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});res.end(JSON.stringify({ok:true,app:'me-ensina-ai-editor',runtimeVersion:'0.1.0',port:actualPort,editorAvailable:true,codexAvailable}));return;
  }
  let index=0;
  const next=()=>{
