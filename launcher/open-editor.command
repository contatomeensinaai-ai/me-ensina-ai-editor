...
release_file="$launcher_dir/release.json"
...
read_release() {
  ...
  release_sha="$(/usr/bin/plutil -extract sha256 raw -o - "$release_file")"
  package_name="$(/usr/bin/plutil -extract packageDirectory raw -o - "$release_file")"
...
if [ "${1:-}" = '--validate-release' ]; then read_release || { echo 'release.json inválido ou SHA ainda não preenchido.' >&2; exit 1; }; echo 'release.json válido'; exit 0; fi
[ "$(/usr/bin/uname -s)" = Darwin ] && [ "$(/usr/bin/uname -m)" = arm64 ] || { echo 'Este piloto requer macOS Apple Silicon.' >&2; exit 1; }
...
if [ ! -x "$package/bin/node" ] || [ ! -f "$package/runtime/server.mjs" ] || { [ ! -f "$package/editor/dist/index.html" ] && [ ! -f "$package/setup-editor.mjs" ]; }; then
  read_release || { echo 'release.json inválido ou SHA ainda não preenchido.' >&2; exit 1; }
  /bin/mkdir -p "$base/releases"
  target="$base/releases/$version-${release_sha:0:16}"
